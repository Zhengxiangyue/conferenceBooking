...
<div id="page-wrapper">
    <div id="myMeeting">
        <div class="row"  >
            <div class="col-lg-12">
                <h1 class="page-header" style="color: #007c8e">我的会议</h1>
                <!--  我的会议包括即将要开的会和正在开的会       -->
            </div>
            ...
            <div v-for="booking in myMeeting" style="display: none" v-show="myMeeting.length>0">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 clearfix text-right">
                    <button @click="cancelMeeting(booking.bid)" class="btn btn-danger btn-outline">取消会议</button>
                </div>
            ...
            <div class="col-ld-12 text-center" style="margin-bottom: 20px;display: none" v-show="true">
                <button :class=" myMeeting.length == total ? 'disabled' : ''" @click="loadMore" class="btn btn-default" style="width: 30%">
                    ...
                    <span v-show="myMeeting.length == total">近期没有会议</span>
                </button>
            </div>
        </div>
    </div>
    ...
    userAppointment_VM = new Vue({
        ...
        methods : {
            getBookingList : function (first = true,hint=false) {
                var self = this;
                $.ajax({
                    url: "/?/api/booking/query/user_future_meeting",
                    type: 'POST',
                    async: true,
                    data : {
                        limit : self.page * 2,
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if(result.code == 1){
                            ...
                            hint_message.show('会议取消成功','success');
